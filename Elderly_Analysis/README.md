...
You can regenerate the analysis and the Folium heatmap by running:

```bash
python3 analyze_elderly_risk.py
```

This will output the statistical findings to the terminal (and save them to `elderly_stats_output.txt`) and build an interactive map titled `Elderly_Involved_Crashes_Map.html` that plots every single crash involving an elderly party.
